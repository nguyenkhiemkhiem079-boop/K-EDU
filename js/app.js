...
async function triggerAutoGenerateDgnlExam() {
  if (typeof MathEngine === 'undefined' || !MathEngine.generateDgnlExam) {
    showToast('⚠️ Bộ sinh đề ĐGNL chưa sẵn sàng, vui lòng thử lại.', 'warn');
    return;
  }

  try {
    const isFull = document.getElementById('dgnlPackageFull')?.checked;
    const packageType = isFull ? 'full' : 'mini';
    const targetExam = document.getElementById('dgnlTargetExamSelect')?.value || 'HCM';
    ...
    const targetName = targetLabels[targetExam] || 'ĐHQG TP.HCM';

    showToast(`⏳ Đang tự động sinh đề ĐGNL ${targetName} (${packageType === 'full' ? '120' : '100'} câu)...`, 'info');

    const generated = MathEngine.generateDgnlExam({
      packageType,
      targetExam,
      difficultyMode: document.getElementById('mathGenDifficultyMode')?.value || 'basic',
      timeLimit
    });

    if (!generated) {
      showToast('⚠️ Không thể sinh đề ĐGNL, vui lòng thử lại.', 'warn');
      return;
    }
    ...
    AppState.teacherFileName = `${(generated.title || 'De_DGNL').replace(/\s+/g, '_')}.html`;
    ...
    if (nameBadge) {
      ...
      nameBadge.innerHTML = `📄 <strong>Đề thi ĐGNL đã sinh:</strong> ${escapeHtml(generated.title)}`;
    }
    ...
    const autoQuiz = {
      ...
      subject: 'toan',
      subjectLabel: 'Đánh giá năng lực',
      difficultyMode: generated.difficultyMode,
      specializedSourceOnly: generated.specializedSourceOnly,
      discipline: 'dgnl'
    };
...
function previewDgnlExamDocument() {
  ...
  if (typeof MathEngine === 'undefined' || !MathEngine.generateDgnlExam) {
    showToast('⚠️ Không thể xem trước đề ĐGNL.', 'warn');
    return;
  }
  const generated = MathEngine.generateDgnlExam({ packageType, targetExam, timeLimit, difficultyMode: document.getElementById('mathGenDifficultyMode')?.value || 'basic' });
  if (!generated || !generated.examHtml) {
    showToast('⚠️ Không thể tạo bản in đề ĐGNL.', 'warn');
    return;
  }
  const w = window.open('', '_blank');
  if (w) {
    w.document.open();
    w.document.write(generated.examHtml);
    w.document.close();
  }
}
    ...
    vactHtml += `
      <div class="vact-quick-nav-bar" style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:0.75rem;padding:0.45rem;background:var(--bg-tertiary);border-radius:var(--radius-md);border:1px solid var(--border-color);position:sticky;top:0;z-index:5;">
        ...
        ${distinctSections.map(sec => {
          ...
          return `<button type="button" class="btn btn-sm btn-secondary" onclick="document.getElementById('vactSectionBanner_${sec}')?.scrollIntoView({behavior:'smooth',block:'start'})" style="font-size:0.72rem;padding:2px 6px;border-radius:var(--radius-sm);font-weight:800;" title="${sectionNames[sec] || sec}">${label} <span style="opacity:0.8;">(${answeredInSec}/${secQuestions.length})</span></button>`;
        }).join('')}
      </div>
    `;
...
async function handleStartFull120Click() {
  ...
  try {
    ...
    if (!examResult.isComplete) {
      showToast(`Đã tạo ${examResult.totalGenerated}/${examResult.totalRequested} câu (chưa đủ 120 câu do giới hạn ngân hàng). Đang bắt đầu làm bài...`, 'warn');
    } else {
      showToast(`Đã tạo thành công bài thi Full V-ACT 120 (${examResult.totalGenerated} câu)!`, 'success');
    }
