/**
 * KhiemEdu Main Application Controller (Split-Screen & Azota Model)
 */
...
const AppState = {
  ...
  teacherPdfData: null,
  teacherFileName: '',
  teacherAnswerKeys: [], // [{ num: 1, type: 'mcq', correct: 'A', score: 0.5 }]
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  updateGamifyBar();
  initAvatars();
  initTeacherAnswerGrid(12); // Default 12 questions
  renderSampleQuizzes();
...
function parseFastAnswerString() {
  ...
  const items = [];
  // Case 1: Format "1A 2B 3C" or "1.A 2-B" or "1:A"
  const regexWithNum = /(\d+)[\s.:-]+([A-D]|Đúng|Sai|\d+)/gi;
  let match;
  ...
  while ((match = regexWithNum.exec(raw)) !== null) {
    ...
    const type = (val === 'ĐÚNG' || val === 'SAI') ? 'truefalse' : (/^[A-D]$/.test(val) ? 'mcq' : 'essay');
    items.push({ num, type, correct: val, score: 0.5 });
  }

  // Case 2: Plain letters "ABCDABCD..."
  ...
  container.innerHTML = AppState.teacherAnswerKeys.map((item, idx) => {
    return `
      <div class="key-grid-item">
        <span style="font-weight:700;color:var(--primary);min-width:60px;">Câu ${item.num}:</span>
        <div style="display:flex;gap:0.35rem;align-items:center;">
          <button type="button" class="bubble-btn ${item.correct === 'A' ? 'selected' : ''}" style="width:32px;height:32px;font-size:0.8rem;" onclick="setTeacherKeyAnswer(${idx}, 'A')">A</button>
          <button type="button" class="bubble-btn ${item.correct === 'B' ? 'selected' : ''}" style="width:32px;height:32px;font-size:0.8rem;" onclick="setTeacherKeyAnswer(${idx}, 'B')">B</button>
          <button type="button" class="bubble-btn ${item.correct === 'C' ? 'selected' : ''}" style="width:32px;height:32px;font-size:0.8rem;" onclick="setTeacherKeyAnswer(${idx}, 'C')">C</button>
          <button type="button" class="bubble-btn ${item.correct === 'D' ? 'selected' : ''}" style="width:32px;height:32px;font-size:0.8rem;" onclick="setTeacherKeyAnswer(${idx}, 'D')">D</button>
          <select style="padding:0.2rem 0.4rem;font-size:0.75rem;width:80px;margin-left:0.25rem;" onchange="changeTeacherKeyType(${idx}, this.value)">
            <option value="mcq" ${item.type === 'mcq' ? 'selected' : ''}>A/B/C/D</option>
    ...
    setTimeout(() => {
      frame.contentDocument.body.innerHTML = `
        <div style="font-family:sans-serif;padding:30px;color:#333;line-height:1.6;">
          <h2 style="color:#4f46e5;">📄 ĐỀ THI TOÁN HỌC (ĐỀ MẪU HỆ THỐNG)</h2>
          <p><em>(Giáo viên chưa đính kèm file PDF riêng, đang hiển thị đề mẫu chuẩn)</em></p>
          <hr/>
          <p><strong>Câu 1:</strong> Nghiệm của phương trình bậc hai $x^2 - 5x + 6 = 0$ là:<br/>A. x = 1 hoặc x = 6 &nbsp;&nbsp;&nbsp; B. x = 2 hoặc x = 3 &nbsp;&nbsp;&nbsp; C. x = -2 &nbsp;&nbsp;&nbsp; D. x = 0</p>
          <p><strong>Câu 2:</strong> Tính giá trị của biểu thức $P = \\sqrt{16} + \\sqrt[3]{27} - 2^3$:<br/>A. -1 &nbsp;&nbsp;&nbsp; B. 1 &nbsp;&nbsp;&nbsp; C. 7 &nbsp;&nbsp;&nbsp; D. 15</p>
          <p><strong>Câu 3:</strong> Trong mặt phẳng tọa độ Oxy, đồ thị hàm số $y = 2x - 4$ đi qua điểm A(2;0). Đúng hay Sai?</p>
          <p><strong>Câu 4:</strong> Cho tam giác vuông có hai cạnh góc vuông là 3cm và 4cm. Tính cạnh huyền.</p>
        </div>
      `;
    }, 200);
  }

  // Render OMR Answer Sheet
...
async function submitStudentExam(isAuto = false) {
  ...
  keys.forEach((k) => {
    ...
    reviewData.push({
      ...
      isCorrect,
      scoreEarned: isCorrect ? (k.score || 1) : 0
    });
  });

  const total = keys.length;
  // Calculate scaled score on 10-point scale
  const finalScore10 = Math.round((correctCount / total) * 100) / 10;
...
function renderExamResultHero(result, rewards) {
  document.getElementById('resultScoreVal').textContent = `${result.score10}/10`;
...
async function refreshLiveLeaderboard(quizId, className) {
  ...
  box.innerHTML = classResults.slice(0, 6).map((r, i) => `
    <div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px dashed var(--border-color);font-size:0.85rem;">
      ...
      <span style="font-weight:700;color:var(--primary);">${r.score10}đ</span>
    </div>
  `).join('');
}
...
async function loadTeacherResults() {
  ...
  const avgScore = (filtered.reduce((acc, r) => acc + (r.score10 || (r.scorePct / 10)), 0) / totalSubmissions).toFixed(1);
  const highestScore = Math.max(...filtered.map(r => r.score10 || (r.scorePct / 10)));
  const passCount = filtered.filter(r => (r.score10 || (r.scorePct / 10)) >= 5).length;
  ...
  Object.values(byClass).forEach(arr => arr.sort((a, b) => (b.score10 || 0) - (a.score10 || 0) || a.timeTakenSeconds - b.timeTakenSeconds));
            ...
            <tbody>
              ${byClass[className].map((r, i) => `
                <tr>
                  ...
                  <td><strong style="color:${(r.score10 || 0) >= 8 ? 'var(--emerald)' : ((r.score10 || 0) >= 5 ? 'var(--primary)' : 'var(--rose)')};font-size:1.05rem;">${r.score10 || (r.scorePct / 10)}đ</strong></td>
                  ...
                  <td><span class="badge-status ${(r.score10 || 0) >= 5 ? 'badge-pass' : 'badge-fail'}">${(r.score10 || 0) >= 5 ? 'ĐẠT' : 'CHƯA ĐẠT'}</span></td>
                </tr>
              `).join('')}
            </tbody>
          </table>
...
function exportResultsToCsv(quizCode) {
  StorageEngine.getResultsByQuiz(quizCode).then(results => {
    ...
    results.forEach(r => {
      csv += `"${r.name}","${r.className}","${r.quizId}","${r.score10 || (r.scorePct / 10)}","${r.correct}","${r.total}","${r.timeTakenSeconds}","${r.tabSwitches}","${r.submittedAt}"\n`;
    });
