...
function switchMathGenMode(mode = 'standard') {
  ...
  if (mode === 'dgnl') {
    ...
    showToast('🧠 Đã chuyển sang chế độ tạo đề Đánh Giá Năng Lực (Mini 30 / 60 / 100 / Full 120)!', 'info');
  } else {
    if (btnStandard) {
...
const VACT_TEACHER_PROFILE_UI = Object.freeze({
  vact_mini_30: { cardId: 'cardDgnlMini30', radioId: 'dgnlPackageMini30', icon: '⚡', name: 'Mini V-ACT 30', total: 30, minutes: 40, subtitle: 'Luyện nhanh K-EDU' },
  vact_mini_60: { cardId: 'cardDgnlMini60', radioId: 'dgnlPackageMini60', icon: '🎯', name: 'Mini V-ACT 60', total: 60, minutes: 75, subtitle: 'Luyện tập K-EDU' },
  vact_mini_100: { cardId: 'cardDgnlMini100', radioId: 'dgnlPackageMini100', icon: '🧠', name: 'Mini V-ACT 100', total: 100, minutes: 90, subtitle: 'Luyện tập đầy đủ' },
  vact_full: { cardId: 'cardDgnlFull', radioId: 'dgnlPackageFull', icon: '🏆', name: 'Full V-ACT 120', total: 120, minutes: 150, subtitle: 'Mô phỏng đầy đủ' }
});

function getSelectedDgnlProfileId() {
  return document.querySelector('input[name="dgnlPackageRadio"]:checked')?.value || 'vact_mini_30';
}

function selectDgnlPackage(profileId = 'vact_mini_30') {
  const legacyAliases = { mini: 'vact_mini_100', full: 'vact_full' };
  const canonicalId = legacyAliases[profileId] || profileId;
  const selected = VACT_TEACHER_PROFILE_UI[canonicalId] || VACT_TEACHER_PROFILE_UI.vact_mini_30;
  const timeInput = document.getElementById('dgnlTimeLimitInput');
  const btn = document.getElementById('btnGenerateDgnlExam');

  Object.entries(VACT_TEACHER_PROFILE_UI).forEach(([id, profile]) => {
    const radio = document.getElementById(profile.radioId);
    const card = document.getElementById(profile.cardId);
    const active = id === canonicalId;
    if (radio) radio.checked = active;
    if (card) {
      card.style.border = active ? '2.5px solid var(--indigo)' : '2px solid var(--border-color)';
      card.style.opacity = active ? '1' : '0.85';
      card.style.boxShadow = active ? '0 4px 14px rgba(99,102,241,0.2)' : 'none';
    }
  });

  if (timeInput) timeInput.value = selected.minutes;
  if (btn) btn.innerHTML = `🚀 TẠO ${selected.name.toUpperCase()} — ${selected.total} CÂU — ${selected.minutes} PHÚT`;
}
...
async function triggerAutoGenerateDgnlExam() {
  ...
  try {
    const profileId = getSelectedDgnlProfileId();
    const profileUi = VACT_TEACHER_PROFILE_UI[profileId] || VACT_TEACHER_PROFILE_UI.vact_mini_30;
    const timeLimit = parseInt(document.getElementById('dgnlTimeLimitInput')?.value || String(profileUi.minutes), 10) || profileUi.minutes;

    showToast(`⏳ Đang tự động sinh ${profileUi.name} (${profileUi.total} câu)...`, 'info');
    ...
    const examResult = examGen.generateFromProfile(profileId, { timeLimitMinutes: timeLimit });
    ...
    // 3. Tạo preview HTML
    const dataUrl = generated.pdfDataUrl && !generated.pdfDataUrl.startsWith?.('data:text/html') ? generated.pdfDataUrl : null;
    ...
    if (previewFrame) await window.ExamDocumentRenderer.renderIntoFrame(previewFrame, { ...generated, pdfDataUrl: dataUrl }, { audience: 'teacher' });
          ...
          <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:0.75rem;">
            <div>
              ...
              <div style="font-size:0.875rem;font-weight:700;color:var(--emerald-shadow);">
                🧠 ${profileUi.name} · 100% Trắc nghiệm chuẩn hóa (${generated.mcqCount} câu) · Thời gian: ${generated.timeLimit} phút · Mã đề: <strong>${newQuizId}</strong>
              </div>
            </div>
    ...
    showToast(`🎉 Đã tạo và phát hành ${profileUi.name} [${newQuizId}] thành công!`, 'success');
  } catch (err) {
...
async function triggerAutoGenerateMathExam() {
  ...
  try {
    ...
    if (previewWrap) previewWrap.classList.remove('hidden');
    if (previewFrame) await window.ExamDocumentRenderer.renderIntoFrame(previewFrame, generated, { audience: 'teacher' });
    if (clearBtn) clearBtn.classList.remove('hidden');
    if (nameBadge) {
      nameBadge.classList.remove('hidden');
...
async function editTeacherQuiz(quizId) {
  ...
  // Handle preview through the canonical document renderer.
  let pdfData = quiz.pdfDataUrl || null;
  ...
  const previewState = await window.ExamDocumentRenderer.resolveQuizDocument(quiz);
  if (previewState.success) {
    if (previewWrap) previewWrap.classList.remove('hidden');
    if (previewFrame) await window.ExamDocumentRenderer.renderIntoFrame(previewFrame, quiz, { audience: 'teacher' });
    if (clearBtn) clearBtn.classList.remove('hidden');
    if (nameBadge) {
      nameBadge.classList.remove('hidden');
...
async function startExamWithQuizId(quizId) {
  ...
  const documentState = await window.ExamDocumentRenderer.resolveQuizDocument(quiz);
  if (!documentState.success) {
    statusEl.innerHTML = '<span style="color:var(--rose);">⚠️ Không tìm thấy nội dung đề thi. Vui lòng thử lại hoặc liên hệ giáo viên.</span>';
    return;
  }
  ...
  const renderResult = await window.ExamDocumentRenderer.renderIntoFrame(frame, quiz, { audience: 'student' });
  if (!renderResult.success) {
    statusEl.innerHTML = '<span style="color:var(--rose);">⚠️ Không thể hiển thị đề thi.</span>';
    return;
  }
